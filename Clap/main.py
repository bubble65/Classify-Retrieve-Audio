...
from SimCLR import ContrastiveTransformations, audio_contrastive_transforms, SimCLR
...
parser.add_argument("--method", type=str, default="mel")
...
class AudioDataset(Dataset):
    def __init__(
        self,
        root_dir="/Users/writemath/课程学习/大学课件/大三上/数字信号处理/大作业/ESC-50-master/audio",
        fold=None,
        feature_type="mfcc",
        config=None,
    ...
    def _cal_features(self):
        ...
        for idx in tqdm(
            ...
            try:
                ...
                if mode == 'CLR':
                    waves = audio_contrastive_transforms( waveform, sr, n_views=2)
                    features = [self.extract_features(wave, sr) for wave in waves]
                else:
                    features = self.extract_features(waveform, sr)
                # print(features[0].shape, features[1].shape)
                all_features.append([f.numpy() for f in features]   )
    ...
    def extract_features(
        ...
        if self.feature_type == "raw":
            ...
            features = librosa.feature.mfcc(
                y=waveform, sr=sample_rate, n_mfcc=13, n_mels=40, n_fft=2048, hop_length=1024
            )
            features = features.T
        elif self.feature_type == "mel":
            features = librosa.feature.melspectrogram(
                y=waveform, sr=sample_rate, n_mels=40, n_fft=2048, hop_length=1024
            )
            features = features.T
        else:
