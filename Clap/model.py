...
class SimpleVit(nn.Module):
    def __init__(self, num_classes=50):
        ...
        self.vit = torch.hub.load("facebookresearch/dino:main", "dino_vit_small16")
        self.vit.head = nn.Linear(self.vit.head.in_features, num_classes)
        self.vit.conv_proj = nn.Conv2d(1, 3, kernel_size=3, stride=1, padding=1)
    def forward(self, x):
        return self.vit(x)  
    ...
    def extract_features(self, x):
        ...
        x = self.vit.blocks(x)
        x = self.vit.norm(x)
        x = x[:, 0]  # Extract features from the cls token
        return x
